...
<div class="notes">
We will get to see some detail for a couple different times of models, even though I can't cover all `tidymodels` has to offer. I hope it will excite you enough to continue learning about it on your own. 
</div>
...
<div class="notes">
We will walk through all of these machine learning steps, using different `tidymodels` functions along the way. 
</div>
...
<div class="notes">
We will use the `house_prices` dataset from the `moderndive` library. The data were orginally from Kaggle. This dataset contains house sale prices for King County, which includes Seattle. It includes homes sold between May 2014 and May 2015. 
</div>
...
Things I noticed and how I might correct them:
...
* I used a few `step_xxx` functions, with brief descriptions in the code. For example `step_log()` takes the log (in this case, base 10) of the variables and `step_dummy()` creates indicator or dummy variables for categorical variables.

* We also use `update_roles()` to change the roles of some variables. 

</div>

## Apply `recipe` and `step`s

Apply recipe and steps to training dataset, just to see the output. Notice the names of the variables.
...
<div class="notes">
Even though it's not necessary, I like applying these to the training data, just to check that all went as planned. Notice the variable `sqft_living` is now actually log base 10 of `sqft_living` and if we scroll to the right we can see examples of the indicator/dummy variables that were created.
</div>
...
* Define the model type, which is the general type of model you want to fit. In this case `linear_reg()`.  
* Set the engine, which defines the package/function that will be used to fit the model.  In this case "lm".  
* Set the mode, which is either "regression" for continuous response variables or "classification" for binary/categorical response variables. (Note that for linear regression, it can only be "regression", so we don't NEED this step in this case.)  
* (OPTIONAL) Set arguments to tune. We'll see an example of this later.
...
<div class="notes">
Start the workflow, add the recipe and the model.
</div>
...
<div class="notes">
Thankfully this is easy to implement with the `fit_resamples()` function. The column called `.estimate` in the output shows the rmse for each fold. They are similar but not all the same. 
</div>
...
<div class="notes">
The `collect_metrics()` function averages the metrics over all the evaluation folds. I also provided the code to show you how you could do that "by hand".
</div>
...
```
<div class="notes">
In this simple scenario, we may be interested in seeing how the model performs on the testing data that was left out. The the `last_fit()` function will fit the model to the training data and apply it to the testing data. Because it is using the workflow, it is applying the same transformation steps to the testing data as were applied to the training data.
...
When we use create models, it is important to think about how the model will be used and specifically how the model could do harm. One thing to notice in the graphs above is that the lower priced homes are, on average, overestimated whereas higher priced homes are, on average, underestimated. 
...
<div class="notes">
We can see that we are using a different model now.
</div>
...
<div class="notes">
Then look at the cross-validated results. This shows rmse for fold 1 for each `penalty` value from `penalty_grid`. The model is fit to data from folds 2-5, then applied to fold 1 and RMSE is computed.
</div>
...
<div class="notes">
Thank you to all of these folks for the wonderful resources and for developing these packages!
</div>
